...
approach the $1.93\times$ advantage of the deterministic pursuer. The remaining
...
(Option~C's +21--22\%) have less statistical support.
...
condition experiments (Exps~20--24) use 5 seeds each. Exp~22 (2 shelves, 5$\times$5)
includes all 5 seeds in the OOD metrics reported here, but seed~42's in-distribution
training layout is structurally degenerate — the package source or destination is
inaccessible under the 2-shelf configuration, so neither policy delivers a single
package in-distribution for that seed. The OOD layouts do not share this property
and the OOD metrics are valid.
