...
Crucially, the 
protocols also deliver far fewer packages in absolute terms. OOD A$\to$N drops from 151.8 (Exp~1) to 67.6 (Exp~10: deterministic) despite having more training time, and from 89.6 (Exp~9) to 49.8 (Exp~11: learned). The proximity encoding removes too much spatial structure for the agent to learn
...
9 octant bins). State space: $1{,}875 \times 81 = 151{,}875$. Uunlike raw coordinate offsets, octant direction uses a compact 81-value encoding that keeps the state space coverable while still providing directional signal (moreso than proximity encoding).

\textbf{Results.} OOD N$\to$N improves from 95.8 to \textbf{115.8} a \textbf{+21\% gain} over the baseline, with ID performance maintained
at $\sim$48--49/50. OOD collisions remain high ($\sim$386/500), likely because the 81$\times$ state expansion limits coverage density.

Interestingly, the improvement does not apply to both adversarial protocols. With a learned adversary OOD A$\to$N rises from
89.6 (baseline) to 118.4 but for a deterministic adversary OOD A$\to$N falls from 151.8 to 121.2. For the adversary-trained policy, it hurts the
  deterministic variant (whose A→N advantage was driven by coverage which the 81× expansion dilutes) and helps the learned variant (which hadn't been getting the same coverage benefit from the
  pursuer). The net effect is that both experiments converge to similar A→N and N→N
  values, which produces the near-1.0 ratios.
...
\midrule
Exp~9: co-evolving (single-layout)   & 95.8  & 89.6  & 0.93$\times$ & $\sim$310 \\
Exp~18: freeze@10k (single-layout)  & 90.5  & 90.5  & \textbf{1.00$\times$} & $\sim$310 \\
Exp~15: multi-layout (no freeze)    & 214.8 & 214.6 & 1.00$\times$ & $\sim$23  \\
Exp~19: multi-layout + freeze@20k   & 218.3 & 224.0 & \textbf{1.03$\times$} & $\sim$23  \\
\bottomrule
\end{tabular}
\end{table}
...
The boundary of the OOD benefit is not determined by added complexity in general. Across varied configurations (more packages, more shelves, more adversaries, larger
...
degrades for both policies equally. Table~\ref{tab:boundary} shows four main outcomes:
...
episodes. Both policies operate near-randomly on unseen OOD joint states, eliminating the advantage.

\textbf{Moderate benefit (Exp~24: $1.26\times$, 4/5 seeds):} The 7$\times$7 grid with 2 shelves preserves a meaningful OOD advantage. The larger grid provides enough
navigable space that adversary training still produces generalizable positional
diversity.

\textbf{Near-neutral (Exps~22, 23):} Two shelves on 5$\times$5 ($1.07\times$) and two adversaries ($1.06\times$) both produce slight, inconsistent
benefits. Neither configuration definitively helps or hurts.

The key variable governing failure is state space size relative to training budget. Exp~21's 62{,}500-state joint space is the only configuration where coverage collapses entirely. The obstacle density result (Exps~22 vs~24) shows that larger grids preserve the benefit under added obstacles.
