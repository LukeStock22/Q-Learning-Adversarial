...
OOD N$\to$N delivery more than doubles (+141\% det, +124\% learned vs.\ baseline). More remarkably,
...
\textbf{Results.} OOD N$\to$N: \textbf{183.0} (det) and \textbf{178.6} (la\_zs) —
better than C alone (115.8/116.8) but substantially worse than D alone (230.8/214.8).
OOD collisions jump to $\sim$318/500, close to C-alone's $\sim$386/500 and far from
D-alone's $\sim$23--28/500.

In absolute terms, both protocols still improve over baseline: OOD A$\to$N rises from
89.6 to 173.4 (det) and 172.0 (learned). The combination is not a failure in the sense
...
C: coarse dir.     & $\sim$386 & 116  \\
D: multi-layout    & $\sim$23--28 & 223  \\
C+D: combined      & $\sim$318 & 181  \\
\bottomrule
\end{tabular}
\end{table}
...
$1.93\times$ persists: the quality of the partially-trained frozen policy (learned
...
Exps~10--11: proximity          & 375     & 48--56    & $\sim$270 & $\sim$43/50 \\
Exps~12--13: coarse dir.        & 151,875 & 116--117  & $\sim$386 & $\sim$48/50 \\
\textbf{Exps~14--15: multi-layout} & \textbf{1,875} & \textbf{215--231} & $\boldsymbol{\sim}$\textbf{23--28} & $\sim$42/50 \\
Exps~16--17: coarse + multi     & 151,875 & 179--183  & $\sim$318 & $\sim$48/50 \\
\bottomrule
\end{tabular}
\end{table}
...
Having established that the deterministic adversary provides a 1.93$\times$ OOD delivery
...
\caption{Boundary conditions: OOD A$\to$N / N$\to$N delivery ratio across five
         ...
         Baseline (Exp~1) = 1.93$\times$ (6 seeds, no reversals).}
\label{tab:boundary}
\centering
\footnotesize
...
\midrule
Exp 1 (baseline)  & ---                         & 1,875  & 1.93$\times$ & 6/6 & Benefit \\
Exp 3 (7$\times$7 baseline) & 7$\times$7 grid  & 7,203  & 3.01$\times$ & 5/5 & Stronger \\
\midrule
...
The benefit is reduced compared to single-agent (1.93$\times$) due to sparser
...
\midrule
Exp 3 (Sec.~\ref{sec:main}) & 5 & 56.6 & 170.4 & 3.01$\times$ \\
